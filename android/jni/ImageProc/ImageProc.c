...
void processimage(const void *p, int l) {
	...
	my_jpeg_load_dht(&mycinfo, my_jpeg_odml_dht, mycinfo.ac_huff_tbl_ptrs,
	...
	if (jpegbuffer == NULL) {
		jpegbuffer = (*mycinfo.mem->alloc_sarray)((j_common_ptr) &mycinfo,
...
int Java_org_x3f_autobot_MjpegInputStream_pixeltobmp(JNIEnv* env, jobject thiz,
...
void Java_org_x3f_autobot_MjpegInputStream_freeCameraMemory(JNIEnv* env,
		jobject thiz) {
